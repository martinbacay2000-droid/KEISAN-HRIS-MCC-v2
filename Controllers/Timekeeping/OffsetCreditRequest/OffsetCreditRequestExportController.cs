...
namespace KEISAN_HRIS_v2.Controllers.Timekeeping.OffsetCreditRequest
{
    ...
    public class OffsetCreditRequestExportController : BaseController
    {
        private readonly IDbConnection _db;

        public OffsetCreditRequestExportController(IDbConnection db) => _db = db;

        // Exports offset credit request data to Excel based on specified filters and returns the file for download
        [HttpGet]
            ...
            string? sortColumn = null, string? sortDirection = "asc")
        {
            try
            {
                ...
                var data = GetOffsetCreditRequestData(status, branch, department, dateFrom, dateTo,
                ...
                var fileName = $"OffsetCreditRequest_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        // Retrieves offset credit request records from the database with applied filters and pagination
        private List<Dictionary<string, object>> GetOffsetCreditRequestData(string? status, string? branch,
            ...
            SELECT 
                ...
                rq.approvedRenderOT AS 'Approved Render OT',
                IFNULL(rq.overTimeReason, '') AS 'Reason',
        ...
        private byte[] GenerateExcelFile(List<Dictionary<string, object>> data, (string EmployeeNo, string EmployeeName) employeeInfo)
        {
            ...
            var ws = package.Workbook.Worksheets.Add("Offset Credit Requests");
            ...
            ws.Cells[1, 1].Value = "Offset Credit Requests";
