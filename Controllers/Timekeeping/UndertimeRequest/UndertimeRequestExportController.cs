...
namespace KEISAN_HRIS_v2.Controllers.Timekeeping.UndertimeRequest
{
    ...
    public class UndertimeRequestExportController : BaseController
    {
        private readonly IDbConnection _db;

        public UndertimeRequestExportController(IDbConnection db) => _db = db;

        // Exports undertime request data to Excel based on specified filters and returns the file for download
        [HttpGet]
            ...
            string? dateFrom, string? dateTo, int offset = 0, int limit = -1)
        {
            try
            {
                ...
                var data = GetUndertimeRequestData(status, branch, department, dateFrom, dateTo, offset, limit);

                if (data.Count == 0)
                    return BadRequest(new { success = false, message = "No data to export" });

                var excelFile = GenerateExcelFile(data, employeeInfo);
                var fileName = $"UndertimeRequest_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        // Retrieves undertime request records from the database with applied filters and pagination
        private List<Dictionary<string, object>> GetUndertimeRequestData(string? status, string? branch,
            string? department, string? dateFrom, string? dateTo, int offset, int limit)
        {
            var query = new StringBuilder(@"
                SELECT 
                    ...
                    DATE_FORMAT(rq.undertimeDateIN, '%Y-%m-%d') AS 'Date In',
                    DATE_FORMAT(rq.undertimeDateOUT, '%Y-%m-%d') AS 'Date Out',
                    TIME_FORMAT(rq.undertimeTimeOUT, '%H:%i') AS 'Time Out',
                    rq.undertimeReason AS 'Reason',
            ...
            string? status, string? branch, string? department, string? dateFrom, string? dateTo)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dateFrom) && !string.IsNullOrWhiteSpace(dateTo))
            {
                ...
                query.Append(" AND DATE(rq.undertimeDateIN) BETWEEN @dateFrom AND @dateTo");
        ...
        private byte[] GenerateExcelFile(List<Dictionary<string, object>> data, (string EmployeeNo, string EmployeeName) employeeInfo)
        {
            ...
            var ws = package.Workbook.Worksheets.Add("Undertime Requests");
            ...
            ws.Cells[1, 1].Value = "Undertime Requests";
