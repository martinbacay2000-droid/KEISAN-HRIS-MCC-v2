...
namespace KEISAN_HRIS_v2.Controllers.Timekeeping.ObRequest
{
    ...
    public class OBRequestExportController : BaseController
    {
        private readonly IDbConnection _db;

        public OBRequestExportController(IDbConnection db) => _db = db;
            ...
            string? sortColumn = null, string? sortDirection = "asc")
        {
            try
            {
                ...
                var data = GetOBRequestData(status, branch, department, dateFrom, dateTo,
                ...
                var fileName = $"OBRequest_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        private List<Dictionary<string, object>> GetOBRequestData(string? status, string? branch,
            ...
            string? sortColumn, string? sortDirection)
        {
            var query = new StringBuilder(@"
                SELECT 
                    ...
                    DATE_FORMAT(rq.obDateIn, '%Y-%m-%d') AS 'Date In',
                    TIME_FORMAT(rq.obTimeIn, '%H:%i') AS 'Time In',
                    DATE_FORMAT(rq.obDateOut, '%Y-%m-%d') AS 'Date Out',
                    TIME_FORMAT(rq.obTimeOut, '%H:%i') AS 'Time Out',
                    rq.obReason AS 'Reason',
            ...
            string? status, string? branch, string? department, string? dateFrom, string? dateTo)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dateFrom) && !string.IsNullOrWhiteSpace(dateTo))
            {
                ...
                query.Append(" AND DATE(rq.obDateIn) BETWEEN @dateFrom AND @dateTo");
        ...
        private byte[] GenerateExcelFile(List<Dictionary<string, object>> data, (string EmployeeNo, string EmployeeName) employeeInfo)
        {
            ...
            var ws = package.Workbook.Worksheets.Add("OB Requests");
            ...
            ws.Cells[1, 1].Value = "Official Business Requests";
