...
namespace KEISAN_HRIS_v2.Controllers.Timekeeping.WFHRequest
{
    ...
    public class WFHRequestExportController : BaseController
    {
        private readonly IDbConnection _db;

        public WFHRequestExportController(IDbConnection db)
        {
            _db = db;
        }
            ...
            string sortDirection = "desc")
        {
            try
            {
                ...
                var data = GetWFHRequestData(status, branch, department, dateFrom, dateTo,
                ...
                var fileName = $"WFH_Requests_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        // Retrieves WFH request records from the database with applied filters and pagination
        private List<Dictionary<string, object>> GetWFHRequestData(string status, string branch,
            ...
            string sortColumn, string sortDirection)
        {
            var query = new StringBuilder(@"
                SELECT 
                    ...
                    DATE_FORMAT(rq.wfhDateIn, '%Y-%m-%d') AS 'Date In',
                    TIME_FORMAT(rq.wfhTimeIn, '%H:%i') AS 'Time In',
                    DATE_FORMAT(rq.wfhDateOut, '%Y-%m-%d') AS 'Date Out',
                    TIME_FORMAT(rq.wfhTimeOut, '%H:%i') AS 'Time Out',
        ...
        private string GetColumnKey(string sortColumn)
        {
            return sortColumn?.ToLower() switch
            {
                ...
                "displaydatein" => "Date In",
                "displaydateout" => "Date Out",
            ...
            string status, string branch, string department, string dateFrom, string dateTo)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dateFrom) && !string.IsNullOrWhiteSpace(dateTo))
            {
                ...
                query.Append(" AND DATE(rq.wfhDateIn) BETWEEN @dateFrom AND @dateTo");
        ...
        private byte[] GenerateExcelFile(List<Dictionary<string, object>> data, (string EmployeeNo, string EmployeeName) employeeInfo)
        {
            ...
            var ws = package.Workbook.Worksheets.Add("WFH Requests");
            ...
            ws.Cells[1, 1].Value = "Work From Home Requests";
        ...
        // Applies color coding to status cells based on status value
        private void ApplyStatusColor(ExcelRange cell, string status)
        {
            ...
            var (bgColor, fgColor) = status.ToUpper() switch
            {
                ...
                _ => (System.Drawing.Color.FromArgb(255, 242, 204), System.Drawing.Color.FromArgb(156, 101, 0)) // Pending
            };
