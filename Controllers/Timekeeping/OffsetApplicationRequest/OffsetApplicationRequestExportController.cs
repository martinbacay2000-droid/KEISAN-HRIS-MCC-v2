...
namespace KEISAN_HRIS_v2.Controllers.Timekeeping.OffsetApplicationRequest
{
    ...
    public class OffsetApplicationRequestExportController : BaseController
    {
        private readonly IDbConnection _db;

        public OffsetApplicationRequestExportController(IDbConnection db) => _db = db;

        // Exports offset application request data to Excel based on specified filters and returns the file for download
        [HttpGet]
            ...
            string? sortColumn = null, string? sortDirection = "asc")
        {
            try
            {
                ...
                var data = GetOffsetApplicationRequestData(status, branch, department, dateFrom, dateTo,
                ...
                var fileName = $"OffsetApplicationRequest_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        // Retrieves offset application request records from the database with applied filters and pagination
        private List<Dictionary<string, object>> GetOffsetApplicationRequestData(string? status, string? branch,
            ...
            WHERE rq.isActive = 1 AND rq.leaveCode = 'CTO'");
        ...
        private byte[] GenerateExcelFile(List<Dictionary<string, object>> data, (string EmployeeNo, string EmployeeName) employeeInfo)
        {
            ...
            var sheetName = "Offset Application Requests";
            ...
            var title = "Offset Application Requests (CTO)";
