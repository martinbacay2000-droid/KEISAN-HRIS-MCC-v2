...
namespace KEISAN_HRIS_v2.Controllers.Timekeeping.ChangeScheduleRequest
{
    ...
    public class ChangeScheduleRequestExportController : BaseController
    {
        private readonly IDbConnection _db;

        public ChangeScheduleRequestExportController(IDbConnection db) => _db = db;

        // Exports change schedule request data to Excel based on specified filters and returns the file for download
        [HttpGet]
            ...
            string? sortColumn = null, string? sortDirection = "asc")
        {
            try
            {
                ...
                var data = GetChangeScheduleRequestData(status, branch, department, dateFrom, dateTo,
                ...
                var fileName = $"ChangeScheduleRequest_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx";
        ...
        // Retrieves change schedule request records from the database with applied filters and pagination
        private List<Dictionary<string, object>> GetChangeScheduleRequestData(string? status, string? branch,
            ...
            string? sortColumn, string? sortDirection)
        {
            var query = new StringBuilder(@"
                SELECT 
                    ...
                    DATE_FORMAT(rq.effectivityDate, '%Y-%m-%d') AS 'Effectivity Date',
                    TIME_FORMAT(rq.timeIN, '%H:%i') AS 'Time In',
                    TIME_FORMAT(rq.timeOUT, '%H:%i') AS 'Time Out',
                    IFNULL(ss.scheduleTypeName, '') AS 'Schedule Type',
                    IFNULL(rq.Reason, '') AS 'Reason',
        ...
        private string GetColumnKey(string sortColumn)
        {
            return sortColumn?.ToLower() switch
            {
                ...
                "displayeffectivitydate" => "Effectivity Date",
            ...
            string? status, string? branch, string? department, string? dateFrom, string? dateTo)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dateFrom) && !string.IsNullOrWhiteSpace(dateTo))
            {
                ...
                query.Append(" AND DATE(rq.effectivityDate) BETWEEN @dateFrom AND @dateTo");
        ...
        private byte[] GenerateExcelFile(List<Dictionary<string, object>> data, (string EmployeeNo, string EmployeeName) employeeInfo)
        {
            ...
            var ws = package.Workbook.Worksheets.Add("Change Schedule Requests");
            ...
            ws.Cells[1, 1].Value = "Change Schedule Requests";
