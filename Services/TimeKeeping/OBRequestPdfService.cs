...
{
    public class OBRequestPdfService
    {
        private readonly string _logoPath;

        public OBRequestPdfService(IWebHostEnvironment env)
        {
        ...
        public byte[] GenerateOBRequestPdf(OBRequestPdfData data)
        {
        ...
        private void ComposeHeader(IContainer container, OBRequestPdfData data)
        {
            container.Column(column =>
            {
                ...
                column.Item().Row(row =>
                {
                    ...
                    {
                        col.Item().AlignCenter().Text("OFFICIAL BUSINESS REQUEST")
                            .FontSize(16).Bold().FontColor("#1e40af");

                        col.Item().AlignCenter().PaddingTop(2).Text("Employee Travel Authorization Form")
                    ...
                    row.ConstantItem(80).AlignRight().Column(col =>
                    {
                        col.Item().Border(1).BorderColor("#e2e8f0")
                            .Background("#f8fafc").Padding(5).Column(innerCol =>
                            {
                                ...
                                innerCol.Item().Text($"OB-{data.Id:D5}").FontSize(11).Bold().FontColor("#1e40af");
                            });
                    });
                });
        ...
        private void ComposeContent(IContainer container, OBRequestPdfData data)
        {
        ...
        private void ComposeEmployeeSection(IContainer container, OBRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
        ...
        // Request Details Section
        private void ComposeRequestDetailsSection(IContainer container, OBRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                column.Item().Background("#eff6ff").Padding(6).Row(row =>
                {
                    row.RelativeItem().Text("TRAVEL DETAILS")
                        .FontSize(9).Bold().FontColor("#1e40af");
                });
                ...
                column.Item().Border(1).BorderColor("#e2e8f0").Padding(10).Column(innerCol =>
                {
                    ...
                    innerCol.Item().Row(row =>
                    {
                        ...
                        {
                            col.Item().Text("Departure").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                        ...
                        row.RelativeItem().Column(col =>
                        {
                            col.Item().Text("Return").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                            col.Item().PaddingTop(3).Row(dateRow =>
                            {
                                dateRow.ConstantItem(60).Text("Date:")
        ...
        private void ComposePurposeSection(IContainer container, OBRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                column.Item().Background("#eff6ff").Padding(6).Row(row =>
                {
                    row.RelativeItem().Text("PURPOSE OF OFFICIAL BUSINESS")
                        .FontSize(9).Bold().FontColor("#1e40af");
                });
        ...
        private void ComposeStatusSection(IContainer container, OBRequestPdfData data)
        {
    ...
    // Data model containing all OB Request information needed for PDF generation
    public class OBRequestPdfData
    {
        ...
        public string ObReason { get; set; } = string.Empty;
