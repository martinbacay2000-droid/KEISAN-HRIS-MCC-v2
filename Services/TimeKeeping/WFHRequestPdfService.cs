...
{
    public class WFHRequestPdfService
    {
        private readonly string _logoPath;

        public WFHRequestPdfService(IWebHostEnvironment env)
        {
        ...
        public byte[] GenerateWFHRequestPdf(WFHRequestPdfData data)
        {
        ...
        private void ComposeHeader(IContainer container, WFHRequestPdfData data)
        {
            container.Column(column =>
            {
                ...
                column.Item().Row(row =>
                {
                    ...
                    {
                        col.Item().AlignCenter().Text("WORK FROM HOME REQUEST")
                            .FontSize(16).Bold().FontColor("#1e40af");

                        col.Item().AlignCenter().PaddingTop(2).Text("Remote Work Authorization Form")
                    ...
                    row.ConstantItem(80).AlignRight().Column(col =>
                    {
                        col.Item().Border(1).BorderColor("#e2e8f0")
                            .Background("#f8fafc").Padding(5).Column(innerCol =>
                            {
                                ...
                                innerCol.Item().Text($"WFH-{data.Id:D5}").FontSize(11).Bold().FontColor("#1e40af");
                            });
                    });
                });
        ...
        private void ComposeContent(IContainer container, WFHRequestPdfData data)
        {
            container.PaddingVertical(10).Column(column =>
            {
                ...
                // WFH Period Details Section
                column.Item().Element(c => ComposeWFHDetailsSection(c, data));

                // WFH Duration Calculation Section
                column.Item().Element(c => ComposeWFHDurationSection(c, data));
        ...
        private void ComposeEmployeeSection(IContainer container, WFHRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
        ...
        // WFH Period Details Section
        private void ComposeWFHDetailsSection(IContainer container, WFHRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                column.Item().Background("#eff6ff").Padding(6).Row(row =>
                {
                    row.RelativeItem().Text("WORK FROM HOME PERIOD")
                        .FontSize(9).Bold().FontColor("#1e40af");
                });
                ...
                column.Item().Border(1).BorderColor("#e2e8f0").Padding(10).Column(innerCol =>
                {
                    ...
                    innerCol.Item().Row(row =>
                    {
                        ...
                        {
                            col.Item().Text("WFH Start").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                        ...
                        row.RelativeItem().Column(col =>
                        {
                            col.Item().Text("WFH End").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                            col.Item().PaddingTop(3).Row(dateRow =>
                            {
                                dateRow.ConstantItem(60).Text("Date:")
        ...
        // WFH Duration Calculation Section
        private void ComposeWFHDurationSection(IContainer container, WFHRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                ...
                {
                    row.RelativeItem().Text("WFH SCHEDULE")
                ...
                column.Item().Border(1).BorderColor("#fbbf24").Padding(10).Row(row =>
                {
                    ...
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().AlignCenter().Text("Work From Home Schedule")
                            .FontSize(8).SemiBold().FontColor("#78350f");
                        col.Item().PaddingTop(5).AlignCenter()
                            .Border(1).BorderColor("#fbbf24")
                            .Background("#fffbeb").Padding(10)
                            .Text(FormatScheduleRange(data))
                            .FontSize(14).Bold().FontColor("#92400e");
                    });
                    ...
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().AlignCenter().Text("Total WFH Days")
                            .FontSize(8).SemiBold().FontColor("#78350f");
                        col.Item().PaddingTop(5).AlignCenter()
                            .Border(1).BorderColor("#fbbf24")
                            .Background("#fffbeb").Padding(10)
                            .Text(CalculateTotalDays(data))
                            .FontSize(16).Bold().FontColor("#92400e");
                    });
                });
                ...
                column.Item().PaddingTop(5).Border(1).BorderColor("#dbeafe")
                    ...
                    .Row(row =>
                    {
                        row.RelativeItem().Text("Note: Employee is authorized to work remotely during the specified period. Standard work hours and productivity expectations remain in effect.")
        ...
        private void ComposeReasonSection(IContainer container, WFHRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                column.Item().Background("#eff6ff").Padding(6).Row(row =>
                {
                    row.RelativeItem().Text("REASON FOR WORK FROM HOME")
                        .FontSize(9).Bold().FontColor("#1e40af");
                });
        ...
        private void ComposeStatusSection(IContainer container, WFHRequestPdfData data)
        {
        ...
        // Helper method to format schedule range
        private string FormatScheduleRange(WFHRequestPdfData data)
        {
            if (string.IsNullOrEmpty(data.DisplayDateIn) || string.IsNullOrEmpty(data.DisplayDateOut))
                return "N/A";

            if (data.DisplayDateIn == data.DisplayDateOut)
                return data.DisplayDateIn;

            return $"{data.DisplayDateIn} - {data.DisplayDateOut}";
        }

        // Helper method to calculate total WFH days
        private string CalculateTotalDays(WFHRequestPdfData data)
        {
            try
            {
                // Parse dates
                if (DateTime.TryParse(data.DisplayDateIn, out DateTime dateIn) &&
                    DateTime.TryParse(data.DisplayDateOut, out DateTime dateOut))
                {
                    int totalDays = (int)(dateOut - dateIn).TotalDays + 1;
    ...
    // Data model containing all WFH Request information needed for PDF generation
    public class WFHRequestPdfData
    {
        ...
        public string WfhReason { get; set; } = string.Empty;
