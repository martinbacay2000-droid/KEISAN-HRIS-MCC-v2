...
{
    public class UndertimeRequestPdfService
    {
        private readonly string _logoPath;

        public UndertimeRequestPdfService(IWebHostEnvironment env)
        {
        ...
        public byte[] GenerateUndertimeRequestPdf(UndertimeRequestPdfData data)
        {
        ...
        private void ComposeHeader(IContainer container, UndertimeRequestPdfData data)
        {
            container.Column(column =>
            {
                ...
                column.Item().Row(row =>
                {
                    ...
                    {
                        col.Item().AlignCenter().Text("UNDERTIME REQUEST")
                            .FontSize(16).Bold().FontColor("#1e40af");

                        col.Item().AlignCenter().PaddingTop(2).Text("Employee Early Departure Authorization Form")
                    ...
                    row.ConstantItem(80).AlignRight().Column(col =>
                    {
                        col.Item().Border(1).BorderColor("#e2e8f0")
                            .Background("#f8fafc").Padding(5).Column(innerCol =>
                            {
                                ...
                                innerCol.Item().Text($"UT-{data.Id:D5}").FontSize(11).Bold().FontColor("#1e40af");
                            });
                    });
                });
        ...
        private void ComposeContent(IContainer container, UndertimeRequestPdfData data)
        {
            container.PaddingVertical(10).Column(column =>
            {
                ...
                column.Item().Element(c => ComposeRequestDetailsSection(c, data));

                // Purpose Section
                column.Item().Element(c => ComposePurposeSection(c, data));
        ...
        private void ComposeEmployeeSection(IContainer container, UndertimeRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
        ...
        // Request Details Section
        private void ComposeRequestDetailsSection(IContainer container, UndertimeRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                column.Item().Background("#eff6ff").Padding(6).Row(row =>
                {
                    row.RelativeItem().Text("UNDERTIME DETAILS")
                        .FontSize(9).Bold().FontColor("#1e40af");
                });
                ...
                column.Item().Border(1).BorderColor("#e2e8f0").Padding(10).Column(innerCol =>
                {
                    ...
                    innerCol.Item().Row(row =>
                    {
                        ...
                        {
                            col.Item().Text("Date In").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                            col.Item().PaddingTop(3).Text(data.DisplayDateIn ?? "N/A")
                                .FontSize(9).FontColor("#0f172a");
                        });
                        ...
                        row.RelativeItem().Column(col =>
                        {
                            col.Item().Text("Date Out").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                            col.Item().PaddingTop(3).Text(data.DisplayDateOut ?? "N/A")
                                .FontSize(9).FontColor("#0f172a");
                        });
                        ...
                        row.RelativeItem().Column(col =>
                        {
                            col.Item().Text("Time Out").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                            col.Item().PaddingTop(3).Text(data.DisplayTimeOut ?? "N/A")
                                .FontSize(9).FontColor("#0f172a");
                        });
                    });
                });
        ...
        // Purpose Section with reason and remarks
        private void ComposePurposeSection(IContainer container, UndertimeRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                column.Item().Background("#eff6ff").Padding(6).Row(row =>
                {
                    row.RelativeItem().Text("REASON FOR UNDERTIME")
                        .FontSize(9).Bold().FontColor("#1e40af");
                });
        ...
        private void ComposeStatusSection(IContainer container, UndertimeRequestPdfData data)
        {
    ...
    // Data model containing all Undertime Request information needed for PDF generation
    public class UndertimeRequestPdfData
    {
