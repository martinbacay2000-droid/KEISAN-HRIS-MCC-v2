...
{
    public class OffsetCreditRequestPdfService
    {
        private readonly string _logoPath;

        public OffsetCreditRequestPdfService(IWebHostEnvironment env)
        {
        ...
        public byte[] GenerateOffsetCreditRequestPdf(OffsetCreditRequestPdfData data)
        {
        ...
        private void ComposeHeader(IContainer container, OffsetCreditRequestPdfData data)
        {
            container.Column(column =>
            {
                ...
                column.Item().Row(row =>
                {
                    ...
                    {
                        col.Item().AlignCenter().Text("OFFSET CREDIT REQUEST")
                            .FontSize(16).Bold().FontColor("#1e40af");

                        col.Item().AlignCenter().PaddingTop(2).Text("Time Credit Application Form")
                    ...
                    row.ConstantItem(80).AlignRight().Column(col =>
                    {
                        col.Item().Border(1).BorderColor("#e2e8f0")
                            .Background("#f8fafc").Padding(5).Column(innerCol =>
                            {
                                ...
                                innerCol.Item().Text($"OC-{data.Id:D5}").FontSize(11).Bold().FontColor("#1e40af");
                            });
                    });
                });
        ...
        private void ComposeContent(IContainer container, OffsetCreditRequestPdfData data)
        {
            container.PaddingVertical(10).Column(column =>
            {
                ...
                // Offset Period Details Section
                column.Item().Element(c => ComposeOffsetDetailsSection(c, data));

                // Credit Calculation Section
                column.Item().Element(c => ComposeCreditCalculationSection(c, data));
        ...
        private void ComposeEmployeeSection(IContainer container, OffsetCreditRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
        ...
        // Offset Period Details Section
        private void ComposeOffsetDetailsSection(IContainer container, OffsetCreditRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                column.Item().Background("#eff6ff").Padding(6).Row(row =>
                {
                    row.RelativeItem().Text("OFFSET PERIOD DETAILS")
                        .FontSize(9).Bold().FontColor("#1e40af");
                });
                ...
                column.Item().Border(1).BorderColor("#e2e8f0").Padding(10).Column(innerCol =>
                {
                    ...
                    innerCol.Item().Row(row =>
                    {
                        ...
                        {
                            col.Item().Text("Offset Start").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                        ...
                        row.RelativeItem().Column(col =>
                        {
                            col.Item().Text("Offset End").FontSize(8).SemiBold()
                                .FontColor("#64748b");
                            col.Item().PaddingTop(3).Row(dateRow =>
                            {
                                dateRow.ConstantItem(60).Text("Date:")
        ...
        // Credit Calculation Section
        private void ComposeCreditCalculationSection(IContainer container, OffsetCreditRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                ...
                {
                    row.RelativeItem().Text("CREDIT CALCULATION")
                ...
                column.Item().Border(1).BorderColor("#a7f3d0").Padding(10).Row(row =>
                {
                    ...
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().AlignCenter().Text("Total Credit Minutes")
                            .FontSize(8).SemiBold().FontColor("#064e3b");
                        col.Item().PaddingTop(5).AlignCenter()
                            .Border(1).BorderColor("#34d399")
                            .Background("#d1fae5").Padding(10)
                            .Text($"{data.OffsetMinutes:F0} minutes")
                            .FontSize(16).Bold().FontColor("#059669");
                    });

                    row.ConstantItem(20);

                    // Converted Hours Display
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().AlignCenter().Text("Equivalent Hours")
                            .FontSize(8).SemiBold().FontColor("#064e3b");
                        col.Item().PaddingTop(5).AlignCenter()
                            .Border(1).BorderColor("#34d399")
                            .Background("#d1fae5").Padding(10)
                            .Text(FormatMinutesToHours(data.OffsetMinutes))
                            .FontSize(16).Bold().FontColor("#059669");
                    });
                    ...
                    row.RelativeItem().Column(col =>
                    {
                        col.Item().AlignCenter().Text("Credit Days")
                            .FontSize(8).SemiBold().FontColor("#064e3b");
                        col.Item().PaddingTop(5).AlignCenter()
                            .Border(1).BorderColor("#34d399")
                            .Background("#d1fae5").Padding(10)
                            .Text(FormatMinutesToDays(data.OffsetMinutes))
                            .FontSize(16).Bold().FontColor("#059669");
                    });
                });
                ...
                column.Item().PaddingTop(5).Border(1).BorderColor("#dbeafe")
                    ...
                    .Row(row =>
                    {
                        row.RelativeItem().Text("Note: Offset credits can be used for future leave applications or time adjustments. Credits are subject to company policy and approval.")
        ...
        private void ComposeReasonSection(IContainer container, OffsetCreditRequestPdfData data)
        {
            container.Column(column =>
            {
                // Section Header
                column.Item().Background("#eff6ff").Padding(6).Row(row =>
                {
                    row.RelativeItem().Text("REASON FOR OFFSET CREDIT")
                        .FontSize(9).Bold().FontColor("#1e40af");
                });
        ...
        private void ComposeStatusSection(IContainer container, OffsetCreditRequestPdfData data)
        {
        ...
        // Helper method to format minutes to hours and minutes
        private string FormatMinutesToHours(double minutes)
        {
            if (minutes <= 0)
                return "0h 0m";
        ...
        // Helper method to format minutes to days (assuming 8 hours per day)
        private string FormatMinutesToDays(double minutes)
        {
            if (minutes <= 0)
                return "0 days";

            double days = minutes / (8 * 60); // 8 hours per day = 480 minutes
    ...
    // Data model containing all Offset Credit Request information needed for PDF generation
    public class OffsetCreditRequestPdfData
    {
        ...
        public string OffsetReason { get; set; } = string.Empty;
