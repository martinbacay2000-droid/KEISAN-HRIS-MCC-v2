...
namespace KEISAN_HRIS_v2.Services.EmployeeProfile
{
    ...
    public class SssContributionRow
    {
        public string? Month { get; set; }  // e.g. "January"
        public string? Year { get; set; }  // e.g. "2026"
        public decimal EEContribution { get; set; }  // deductionSSSemployee
        public decimal ERContribution { get; set; }  // deductionSSSemployer
        public decimal ECContribution { get; set; }  // deductionSSSec
        public decimal TotalContribution { get; set; }
        public decimal SssLoan { get; set; }
    }

    public class SssContributionReportData
    {
        public string? EmployeeNo { get; set; }
        public string? EmployeeName { get; set; }  // FIRSTNAME MIDDLENAME LASTNAME
        public string? GenderPrefix { get; set; }  // "MR.", "MS.", or "MR/MS."
        public string? SssNo { get; set; }  // sssNo from e_payrolldetails
        public string? Purpose { get; set; }  // user-entered purpose (optional)
        public string? ForPeriod { get; set; }  // e.g. "JANUARY - MARCH 2026" (reserved)
        ...
        public List<SssContributionRow> Rows { get; set; } = new();
    }

    // ── PDF Service ──────────────────────────────────────────────────────────────

    /// <summary>
    /// Generates the SSS Certificate of Contributions PDF using QuestPDF.
    ///
    /// Changes from v1:
    ...
    /// </summary>
    public class SssContributionPdfService
    {
        ...
        private const string FooterLine =
            ...
            "     Tel: (63)(2) 8863-7777     www.companyname.com";

        public byte[] Generate(SssContributionReportData data)
        {
                ...
                container.Page(page =>
                {
                    ...
                    page.Content().Column(col =>
                    {
                        ...
                        col.Item().Text(txt =>
                        {
                            ...
                            txt.Justify();
                            txt.Span("This is to certify that we have remitted the SSS contributions of ");
                        ...
                        col.Item().Text(txt =>
                        {
                            ...
                            txt.Span("SSS No. : ").FontSize(11);
                            txt.Span(data.SssNo ?? "N/A").Bold().FontSize(11);
                        });
                        ...
                        col.Item().Table(table =>
                        {
                            table.ColumnsDefinition(cols =>
                            {
                                ...
                                cols.RelativeColumn(2.2f); // EE Contribution
                                cols.RelativeColumn(2.2f); // ER Contribution
                                cols.RelativeColumn(1.5f); // EC
                                cols.RelativeColumn(2.2f); // Total
                                cols.RelativeColumn(2.2f); // SSS Loan
                            });
                            ...
                            table.Header(header =>
                            {
                                ...
                                header.Cell().Element(HeaderCell)
                                    .Text("EC").Bold().FontSize(10).FontFamily(FontName);
                                header.Cell().Element(HeaderCell)
                                    .Text("Total").Bold().FontSize(10).FontFamily(FontName);
                                header.Cell().Element(HeaderCell)
                                    .Text("SSS Loan").Bold().FontSize(10).FontFamily(FontName);
                            });
                            ...
                            foreach (var row in data.Rows)
                            {
                                ...
                                table.Cell().Element(DataCellRight)
                                    .Text($"{row.ECContribution:N2}").FontSize(10).FontFamily(FontName);
                                table.Cell().Element(DataCellRight)
                                    .Text($"{row.TotalContribution:N2}").FontSize(10).FontFamily(FontName);
                                table.Cell().Element(DataCellRight)
                                    .Text($"{row.SssLoan:N2}").FontSize(10).FontFamily(FontName);
                            }
                        });
